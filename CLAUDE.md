...
**A안: 지원 범위를 좁힌 프로토타입.** 모델은 CatVTON 그대로, 되는 범위는 "단색 기본 의류 +
잘 찍은 사진"이고, 안 되는 입력(프린트·흰 옷·특수 질감)은 앱이 미리 경고한다.
GPU는 **Kaggle만** 쓴다(상시 서버 없음, 시연 때 Gradio 공개 링크).
...
| 설정 | 값 | 이유 |
|---|---|---|
| 샘플러 | **DDIM** (저장소 기본) | DPM++는 질감·글자 프린트가 흐려진다. 아래 "번복" 참고 |
| 스텝 | **50** | 30은 약 1.7배 빠르지만 14건 중 1건 형태 실패. P100 170초 / T4 약 110초 |
| guidance | 2.5 (상·하의 공통) | 하의 5.0은 단색 바지를 위장무늬로 망가뜨려 되돌림 |
...
| ~~스텝 **올리기** 30·50~~ | ~~8스텝과 차이 없음~~ → **번복.** DPM++ 안에서 스텝만 올리면 차이가 없는 게 맞지만, **샘플러를 DDIM으로** 바꾸면 질감이 확연히 또렷하다 |

**속도 최적화(DPM++ 8스텝)를 번복한 이유 — 같은 실수 반복 금지.**
"8스텝이 50스텝과 구별 안 된다"는 SSIM과 썸네일로 판단했다. SSIM은 구도·형태를 보지
미세한 질감을 못 본다. 사용자가 "질감이 흐리다"고 지적한 뒤 **원본 해상도로 잘라서** 비교하니
DPM++ 8은 글자 프린트가 번지고 니트 결이 사라져 있었다(`docs/PLAN.md` F-7).
